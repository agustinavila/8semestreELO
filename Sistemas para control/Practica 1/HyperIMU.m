...
block.OutputPort(1).Dimensions       = [block.DialogPrm(1).Data 3];
...
if st(1) == 0
    error('HyperIMU block can only handle discrete sample times');
end

% block.NumDworks = 1;
%   
%   block.Dwork(1).Name            = 'UDP';
...
try
    validateattributes(block.DialogPrm(1).Data, {'numeric'}, {'real', 'scalar', 'nonzero', 'nonnegative'}); % Number of sensors set in HyperIMU
    validateattributes(block.DialogPrm(2).Data, {'numeric'}, {'real', 'scalar', 'nonzero', 'nonnegative'}); % Port number set in HyperIMU
    validateattributes(block.DialogPrm(3).Data, {'numeric'}, {'real', 'scalar', 'nonzero'}); % Sample time
catch %#ok<CTCH>
    error('Simulink:invalidParameter', 'Invalid value for a mask parameter');
end
...
block.OutputPort(1).Data = vec2mat(apparray,3);
